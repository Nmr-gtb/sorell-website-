...
18. **Stats analytics = événements dédupliqués, dénominateur honnête** : tout taux se calcule dans `lib/analytics-stats.ts` (module pur, testé) sur des événements dédupliqués par couple (newsletter_id, recipient_email) — la route pixel `open` déduplique à l'insertion, la route `click` NON (un lecteur qui clique 7 fois affichait 350 %). Les volumes bruts (totalOpens/totalClicks/Top articles) restent non dédupliqués : ce sont des volumes, pas des taux. `recipient_count` absent ou 0 = dénominateur INCONNU → taux `null` affiché « — », jamais la liste de destinataires actuelle ni un `|| 1`. Taux plafonnés à 100 %. La carte « Taux d'ouverture » du dashboard porte sur le dernier ENVOI (`status = sent`), pas sur la dernière newsletter générée (un brouillon en attente donnait « — »).
19. **Chargement du dashboard = 1 Promise.all, formatage au rendu** : ne jamais rechaîner des effets en cascade (`if (isNewUser !== false) return`) ni mettre `lang` dans les deps d'un effet de fetch — stocker les données brutes (frequency/send_day/send_hour) et formater dans un `useMemo`. Les listes ne chargent jamais la colonne `content` (plusieurs Ko/ligne) : contenu à la demande au clic. Skeletons via `components/Skeleton.tsx` (jamais « Chargement... » ni spinner), et un échec de requête Supabase doit produire un état d'erreur distinct de l'état vide (vérifier `.error` systématiquement).
20. **Résiliation Stripe = cancel_at_period_end OU cancel_at** : pour un abonnement en TRIAL, le portail pose `cancel_at` (= fin du trial) et laisse `cancel_at_period_end` à false (cas réel du 13/07/2026, 1er abonné payant). Toute logique de détection de résiliation doit tester LES DEUX champs. Le motif est dans `cancellation_details.feedback/comment` — le remonter, c'est l'info rétention la plus utile.
