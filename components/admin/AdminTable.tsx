          ...
          <thead>
            <tr className="border-b border-[var(--border)] bg-[var(--surface-alt)]">
              {columns.map((col) => (
                <th
                  key={col.key}
                  className={`px-6 py-4 text-left text-[11px] font-semibold uppercase tracking-[0.08em] text-[var(--text-muted)] first:pl-10 last:pr-10 ${col.className || ""}`}
                >
            ...
            {loading
              ? Array.from({ length: loadingRows }).map((_, i) => (
                  <tr key={`skeleton-${i}`} className={i % 2 === 1 ? "bg-[var(--surface-alt)]/30" : ""}>
                    {columns.map((col) => (
                      <td key={col.key} className="px-6 py-5 first:pl-10 last:pr-10">
              ...
              : data.length === 0 ? (
                  ...
                  data.map((item, index) => (
                    ...
                    >
                      {columns.map((col) => (
                        <td key={col.key} className={`px-6 py-4 first:pl-10 last:pr-10 ${col.className || ""}`}>
