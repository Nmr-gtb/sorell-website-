  ...
  for (const config of configs) {
    try {
      ...
      const {
        ...
        { referenceDate: franceTime, model: getModelForPlan(userPlan), maxArticles: resolveServerlessArticleCount(userPlan, config.article_count, getModelForPlan(userPlan)).count }
      );
